...
function lazyLoadImages(_a) {
  var _b = _a === void 0 ? {} : _a,
    ...
    img: ['']
      ...
      _d = _b.container,
      container = _d === void 0 ? '' : _d,
      _e = _b.defaultImg,
      defaultImg = _e === void 0 ? '' : _e;

  var entryContent = document.querySelector(container);
  var items = images.reduce(function (outerItems, _a, index) {
    var _b = _a.name,
        name = _b === void 0 ? '' : _b,
        _c = _a.img,
        img = _c === void 0 ? [] : _c; // Parse image list

    var imgItems = img.reduce(function (innerItems, imgSrc) {
  ...
  if (entryContent) {
    entryContent.innerHTML = box; // Lazy load

    var lazyImages_1 = __spreadArrays(document.querySelectorAll('.m-img-item'));

    var inAdvance_1 = 300;

    var lazyLoad_1 = function lazyLoad_1() {
      lazyImages_1.forEach(function (image) {
        if (image.offsetTop < window.innerHeight + window.pageYOffset + inAdvance_1) {
    ...
    window.addEventListener('scroll', mazey.throttle(lazyLoad_1, 50, {}));
    window.addEventListener('resize', mazey.throttle(lazyLoad_1, 50, {})); // Load default image first.
